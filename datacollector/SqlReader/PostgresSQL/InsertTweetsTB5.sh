#!/bin/bash   
#./PostgresSQL/InsertTweetsTB5.sh "host=204.93.216.157 port=5432 user=elwizara password=${pass} dbname=ElwizaraLIVEDB" "host=127.0.0.1 port=5432 user=tarek password=123 dbname=ElwizaraLIVEDB sslmode=disable" 1000.gz
 
dollarSign='$$'

query1="
    COPY \"TweetsTB\" TO PROGRAM 'gzip > /var/lib/postgresql/tmp/TweetsTB/$3';
    TRUNCATE TABLE \"TweetsTB\";
"
...
# select count(*) from "TweetsTB_0";--"547082"-"548357"-"703574"
# select count(*) from "TweetsTB_1";--"304566"-"304824"-"388807"
# select count(*) from "TweetsTB_2";--"236434"-"236755"-"300711"
# select count(*) from "TweetsTB_3";--"171654"-"171861"-"219616"
# select count(*) from "TweetsTB_4";--"202223"-"202696"-"263695"
# select count(*) from "TweetsTB_5";--"164953"-"165000"-"211575"
# select count(*) from "TweetsTB_100";--"164953"-"211782"-"271138"
# select count(*) from "TweetsTB_588";--"194631"-"195231"-"244632"
# select count(*) from "TweetsTB_700";--"189787"-"189807"-"246429"
# select count(*) from "TweetsTB_980";--"202238"-"202680"-"256412"
# select count(*) from "TweetsTB_998";--"200004"-"200447"-"257822"