#!/bin/bash   
#./PostgresSQL/InsertUsersProfilesTB2.sh "host=204.93.216.157 port=5432 user=elwizara password=${pass} dbname=ElwizaraLIVEDB" "host=127.0.0.1 port=5432 user=tarek password=123 dbname=ElwizaraLIVEDB sslmode=disable" 1530008990.gz 1530008990

dollarSign='$$'

query1="        
    COPY(SELECT * FROM \"UsersProfilesTB\" WHERE \"UpdatedAt\" < $4) 
        TO PROGRAM 'gzip > /var/lib/postgresql/tmp/UsersProfilesTB/$3';"
...
psql "$1" -c "DELETE FROM \"UsersProfilesTB\" WHERE \"UpdatedAt\" < $4;" 
